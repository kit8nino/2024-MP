        ...
        if maze[row][col] == ' ' and (row == 0 or row == rows - 1 or col == 0 or col == cols - 1):
            for cell in path:
                ...
                if maze[path_row][path_col] not in ('+', '*'):
                    ...
                    ZAP_DEL.append((path_row, path_col))  # Запись позиций удаленных запятых
            return
...
find_path(maze, avatar_row, avatar_col)

# Восстановление символов ',' с наложением на '.'
for row, col in ZAP_DEL:
    if maze[row][col] == '.':
        maze[row][col] = 'O'
    else:
        maze[row][col] = ','
...
# Выводим координаты выхода и ключ
print("The exit coordinates are:", key_row, key_col)
print("The key coordinates are:", avatar_row, avatar_col)
