...
class FlyingObject:
    ...
    def print_spherical_coords(self, radar_x, radar_y, radar_z):
        ...
        # для того, чтобы фи был в диапазоне [0, 360)
        if phi < 0:
            phi += 360
        ...
        if thetta < 0:
            thetta += 180
        print(f'Объект: r={r:.2f} м, phi={phi:.2f}°, thetta={thetta:.2f}°')
...
class RadarSystem:
    ...
    def get_object_data(self):
        x_coord = float(input('Введите координату x объекта (м): '))
        y_coord = float(input('Введите координату y объекта (м): '))
        z_coord = float(input('Введите координату z объекта (м): '))
        speed_proj_x = float(input('Введите проекцию скорости объекта по оси x (м/с): '))
        speed_proj_y = float(input('Введите проекцию скорости объекта по оси y (м/с): '))
        speed_proj_z = float(input('Введите проекцию скорости объекта по оси z (м/с): '))
        return x_coord, y_coord, z_coord, speed_proj_x, speed_proj_y, speed_proj_z

    def run(self):
        self.radar.get_spherical_coords(self.objects)
        time = float(input('\nВведите время (с) для отображения новых сферических координат: '))
