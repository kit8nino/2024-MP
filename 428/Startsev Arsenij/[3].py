...
#проверка на правильную позицию ключа
def right_key_position(maze, coord):
...
#выход и вход
start_coord = (0, 1)
exit_coord = (1079, 1918)
print("координаты входа:", start_coord)
print("координаты выхода:", exit_coord)

# задаем координаты ключа 
key_x = 201
key_y = 154
key_сoord = (key_x, key_y)

# проверяем, что ключ находится в доступном месте (в пустой клетке)
if not right_key_position(maze, key_сoord):
    print("неверное расположение ключа, выберите другие координаты")
    exit()
...
print("координаты ключа:", key_сoord)

#используем жадный алгоритм и отмечаем путь точками
greedy_path = greedy_search(maze, start_coord, key_сoord)
if greedy_path:
...
heuristic_weight = 1  
max_steps = 15000  
a_star_path = a_star_search(maze, key_сoord, exit_coord, cost_weight, heuristic_weight, max_steps)
if a_star_path:
