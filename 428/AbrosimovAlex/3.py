...
def maze_input(file_name, maze):
    with open(file_name, 'r', encoding="utf-8") as file_in_maze:
        for line in file_in_maze:
            maze.append(list(line.rstrip()))

maze_input('maze-for-u.txt', my_maze)

x_0, y_0 = 1, 3
x_key, y_key = 1062, 1511
x_exit, y_exit = 1079, 936
...
def deikstra(x_0, y_0, maze):
    cells_to_visit = deque([(x_0, y_0, cost_deikstra(x_0, y_0), [(x_0, y_0)])])
    visited = set()
...
def add_to_visit_list_a_key(list_of_neighbours, path, visited, path_length, max_path_length, cells_to_visit):
    for nx, ny in list_of_neighbours:
        if (nx, ny) in visited:
            continue
...
def find_exit(x_0, y_0, maze, max_path_length):
    cells_to_visit = [(x_0, y_0, cost_a_star(x_0, y_0), [(x_0, y_0)], 0)]
    visited = set()
    ...
    while cells_to_visit:
        ...
        list_of_neighbours = neighbours_current_list(x, y, maze)
        cells_to_visit = add_to_visit_list_a_key(list_of_neighbours, path, visited, path_length, max_path_length, cells_to_visit)
