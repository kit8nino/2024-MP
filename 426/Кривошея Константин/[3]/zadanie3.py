...
def set_avatar_coord(x,y,maze):
    if maze[x][y] != '#':
        print('Координаты автара:', x, y)
        maze[x][y] = 'o'
        avatar_coords = [x, y]
    elif maze[x + 1][y] != '#':
        print('Координаты автара:', x + 1, y, '( стена', x, y, ')')
        maze[x+1][y] = 'o'
        avatar_coords = [x + 1, y]
    elif maze[x][y + 1] != '#':
        print('Координаты автара:', x, y + 1, '( стена', x, y, ')')
        maze[x][y+1] = 'o'
        avatar_coords = [x, y+1]
    elif maze[x - 1][y] != '#':
        print('Координаты автара:', x - 1, y, '( стена', x, y, ')')
...
def set_key_coord(x,y,maze):
    if maze[x][y] != '#':
        maze[x][y] = '*'
        print('Коорд ключа:', x, y)
        key_coords = [x, y]
    elif maze[x+1][y] != '#':
        maze[x+1][y] = '*'
        print('Коорд ключа:', x+1, y, '( стена', x, y, ')')
        key_coords = [x+1, y]
    elif maze[x][y+1] != '#':
        maze[x][y+1] = '*'
        print('Коорд ключа:', x, y+1, '( стена', x, y, ')')
        key_coords = [x, y+1]
    elif maze[x-1][y] != '#':
        maze[x-1][y] = '*'
        print('Коорд ключа:', x-1, y, '( стена', x, y, ')')
...
def set_exit_coords(maze):
    for i in range(len(maze[0])):
        if maze[0][i] == " ":
            print('Коорд 1 выход:', 0,i)
            exit1 = [0,i]
    for i in range(len(maze[-1])):
        if maze[-1][i] == " ":
            print('Коорд 2 выход:', len(maze)-1,i)
            exit2 = [len(maze)-1, i]
